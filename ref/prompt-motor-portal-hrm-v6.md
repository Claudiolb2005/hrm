# Prompt de construcción — Motor + Portal de Supervisión de Nómina HRM (v6)
...
> **Todas las preguntas abiertas quedaron resueltas** (ver §14); las reglas de negocio confirmadas por el cliente están integradas en §5.
> `(a decidir)` = decisión de la junta (arquitectura).
...
8. PUBLICAR   → informe visible EN EL PORTAL (no se exporta a PDF/Word); narrativa III/XIII/resumen con IA (editable)
```
...
```
**Mapa de conceptos (CONFIRMADO — el crosswalk viene EMBEBIDO en el encabezado del acumulado):** en el `ACUMULADO`, cada concepto interno va seguido de su equivalente **CFDI (SAT)** en columna marcada con color de acento ("marrón"). El motor **siembra el `mapa_conceptos` del cliente leyendo el encabezado** (no se arma aparte). Tratamientos confirmados:
- `Despensa(Informativo)` → CFDI 029 Vales de despensa. **Se timbra pero la paga un tercero → informativo, FUERA de la conciliación de dispersión y del neto pagado.**
- `Fondo ahorro empresa` (percepción → CFDI 005) y `Fondo de ahorro Empleado`/`de Empresa` (deducción → CFDI 004 Otros) con igual monto **se cancelan** (no se pagan por nómina).
- `Cancelaciones Prematuras` → deducción de **recuperación** (CFDI 004 Otros); no se paga al colaborador.
- `Pago anticipado a neto` → **deducción interna** que reduce el neto (recupera un anticipo); **NO tiene concepto CFDI propio** (rueda a Total deducciones).
- `TOTAL PREVISION SOCIAL` (IMSS/RCV/Infonavit empresa/Impuesto estatal) → **patronal, informativo**; alimenta VIII/IX/X, no el neto.

**El acumulado además trae, lado a lado, los TOTALES internos y los TOTALES CFDI** (`TOTAL PERCEPCIONES` vs `Total percepciones`, `TOTAL DEDUCCIONES` vs `Total deducciones`, `NETO SUELDO FISCAL` vs `Total`) → **R1 queda semi-precalculada**; y trae **columnas de comprobación** (`Comprobación Percepciones/Deducciones/Neto/Previsión`) que el motor usa como red de seguridad para validar al centavo.
...
Canónico mínimo: `Total percepciones`, `Total otros pagos`, `Total deducciones`, `Total`, `Tipo nomina`, `Fecha pago`, `RFC` (y UUID si está).
```
1. leer plano; mapear por nombre (ignorar columnas extra); exigir el set mínimo
2. para cada CFDI:
     // REGLA DE NETO (confirmada): incluir OTROS PAGOS (Subsidio al Empleo), NO usar la columna "Total"
     total_concil = aCentavos(Total percepciones) + aCentavos(Total otros pagos) - aCentavos(Total deducciones)
     fila = { rfc, total_perc, total_otros, total_ded, total: aCentavos(Total),
...
3. para DISPERSIONES (R2) — REGLA CONFIRMADA (PrimeWireless):
   - dos canales cuentan como pago real: BBVA (interno) + interbancarios/SPEI a otros bancos
   - "PAGO DE NÓMINA" y "TRASPASO NÓMINA BBVA" son EL MISMO DINERO → contar SOLO UNO (nunca sumar ambos) = evita doble conteo
   - rechazos / TEF devuelto: NO suman, se RESTAN; el pago queda pendiente y se rastrea después por el importe pagado al colaborador
   - la salida de banco (gasto/DEBIT) se cuadra contra el neto de nómina; la config_conciliacion del cliente define qué etiquetas de póliza mapean a cada regla
...
| Duplicidad de empleado (CONFIRMADO) | mismo RFC 2 veces en el periodo → **sumar importes, contar al empleado 1 sola vez**. Válido solo en desvinculación (liquidación + finiquito = 2 recibos/CFDIs) | cualquier otro caso → **hallazgo de duplicidad** |
| Cobertura (R6) | cada RFC del acumulado tiene CFDI | listar faltantes (sec III/VII) |
| Insumo faltante (CONFIRMADO) | los archivos pueden venir incompletos/parciales | **las 13 secciones SIEMPRE se generan desde el acumulado**; lo que dependa de un insumo ausente se marca "pendiente de X" y **se notifica al cliente** (no se inventa, no se bloquea todo) |
...
| # | A | B | Tolerancia | Sec |
|---|---|---|---|---|
| R1 | Acumulado NETO/periodo | **CFDI: Σperc + Σotros − Σded** | tolerancia ↓ | II |
| R2 | Acumulado NETO/periodo | Pagos de banco (BBVA **o** traspaso = mismo dinero, contar 1 vez) + SPEI − rechazos | dif justificada por rechazos | II |
| R3 | Acumulado I.S.R. | Visor SAT ISR retenido | tolerancia ↓ | VII |
...
- **Tolerancia CONFIRMADA (redondeo): ≤ $0.50 por empleado (50 centavos)** y **≤ $5.00 en la suma del periodo (500 centavos)**. Parametrizable por cliente. Diferencia mayor → **hallazgo** (no se oculta).
...
- Por cliente cambia: formato de archivos (`plantillas_mapeo`), registros patronales y `mapeo_estado_rp`, `mapa_conceptos`, y `config_conciliacion` (qué etiquetas de póliza mapean a cada regla, tolerancias, contra qué compara dispersiones).
- El `mapa_conceptos` de PrimeWireless **se siembra leyendo el crosswalk embebido en el encabezado del acumulado** (columnas CFDI en marrón). Para clientes nuevos: si el acumulado no trae el crosswalk, la IA lo sugiere y HRM lo confirma una vez.
...
| /periodos/:id/informe | 13 secciones (acordeón + charts + tablas) con el **firmante visible** (Alma Claudia Ramírez Escamilla); editar III/XIII/resumen; publicar/despublicar — **se consulta en el portal, sin descarga PDF/Word** | /periodos/:id |
...
| /periodos/:id/informe | 13 secciones (consulta en el portal, **sin descarga**) | /dashboard |

**Visor** `/r/:token`: un informe en **modo lectura**, sin sidebar de gestión y **sin descarga**.
...
Llave `RFC+NETO+PERIODO` · **neto CFDI = perc + otros pagos − ded** · **dispersión: pago = traspaso (mismo dinero, contar 1 vez) + SPEI − rechazos** · **empleado 2 veces → sumar importes, contar 1 vez** (solo desvinculación) · tolerancia **$0.50/empleado y $5.00/periodo** · finiquitos por RFC/periodo y mes completo · provisión = nómina − despensa + pensión · excluir timbres duplicados · meses con `CASE` · puestos híbrido (prefijo EJECUTIVO DE VENTAS / GERENTE REGIONAL, exacto el resto).
...
- ✅ Multi-cliente real: sí. Ambos perfiles suben archivos: sí.
- ✅ **Tolerancia:** ≤ $0.50 por empleado y ≤ $5.00 en la suma del periodo (§5.7).
- ✅ **Dispersiones (R2):** "pago de nómina" y "traspaso nómina BBVA" = **mismo dinero → contar una sola vez**; SPEI a otros bancos cuenta; rechazos/TEF devuelto se restan y quedan pendientes (§5.3.6).
- ✅ **Regla de neto CFDI:** percepciones + otros pagos − deducciones (§5.3.5).
- ✅ **Conceptos:** crosswalk embebido en el acumulado; despensa (informativo), fondo de ahorro (se cancela), cancelaciones prematuras (recuperación), previsión social (patronal informativo), **pago anticipado a neto = deducción interna sin CFDI propio** (§5.3.3).
- ✅ **Empleado 2 veces en el periodo:** sumar importes, contar 1 vez; válido solo en desvinculación; lo demás = duplicidad (§5.6).
- ✅ **Insumos incompletos:** las 13 secciones siempre se generan desde el acumulado; faltantes → notificar al cliente (§5.6).

- ✅ **Entrega del informe:** SOLO en el portal (sin exportación a PDF/Word); el firmante aparece en la vista del portal.

**Decisión de arquitectura pendiente (no bloquea el diseño, sí la implementación):**
- (a decidir) ¿Dónde corre el parseo pesado — Workflows/Queues, navegador limpio o servicio externo? (§5.12)
